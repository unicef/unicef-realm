...
.init-db:
	...
	psql -c 'DROP DATABASE IF EXISTS test_unicef_realm;' -U postgres -h 127.0.0.1
	psql -c 'DROP DATABASE IF EXISTS unicef_realm;' -U postgres -h 127.0.0.1
	psql -c 'CREATE DATABASE unicef_realm;' -U postgres -h 127.0.0.1
...
clean:
	# cleaning
	@rm -rf ${BUILDDIR} .pytest_cache src/unicef_realm.egg-info dist *.xml .cache *.egg-info .coverage .pytest MEDIA_ROOT MANIFEST .cache *.egg build STATIC
