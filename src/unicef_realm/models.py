...
class UserRoleManager(models.Manager):
    def by_permissions(self, perms_name):
        ...
        for perm_name in perms_name:
            ...
            self = self.filter(
                group__permissions__codename=codename, group__permissions__content_type__app_label=app_label
            )
        return self

    def get_permissions_by_target(self, user, target):
        return self.filter(user=user, target=target).values_list(
            "group__permissions__content_type__app_label", "group__permissions__codename"
        )
