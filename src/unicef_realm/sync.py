...
def load_region():
    ...
    for entry in regions:
        region, created = Region.objects.update_or_create(code=entry[0], defaults={"name": entry[1]})
...
def load_business_area():
    ...
    for entry in data:
        defaults = {
            "name": entry["BUSINESS_AREA_NAME"],
            "country": countries.by_name(entry["BUSINESS_AREA_NAME"]),
            "region": Region.objects.get_or_create(code=entry["REGION_CODE"], defaults={"name": entry["REGION_NAME"]})[
                0
            ],
        }
        area, created = BusinessArea.objects.update_or_create(code=entry["BUSINESS_AREA_CODE"], defaults=defaults)
